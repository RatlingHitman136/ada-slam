...
Nothing is recomputed and no GPU is involved - evo already saved both halves of it (12.3):
`evo/error_array.npy` is the per-pose APE and `evo/alignment_transformation_sim3.npy` is the
Sim(3) that puts the estimate in GT coordinates. Applying the second reproduces the first to
...
def load_arm(root, scene, arm, gt_xyz, keyframes):
    ...
    T = load_alignment(out)
    xyz = est[[row_of[f] for f in frames.tolist()], 1:4] @ T[:3, :3].T + T[:3, 3]
    ref = np.array([gt_xyz[f] for f in frames.tolist()])

    # The whole picture rests on this identity, so check it rather than trust it: it is what
    # says the drawn geometry and the drawn colour describe the same alignment.
    drift = float(np.abs(np.linalg.norm(xyz - ref, axis=1) - err).max())
    if drift > 1e-6:
        raise SystemExit(f'{out}: applying evo\'s own Sim(3) to traj_full.txt disagrees with '
                         f'evo/error_array.npy by {drift:.3g} m. One of them is stale - delete '
    ...
    if keyframes:
        ...
        xyz, err, frames = xyz[keep], err[keep], frames[keep]

    res = json.load(open(f'{out}/{RESULTS}')) if os.path.exists(f'{out}/{RESULTS}') else {}
    # `shown` is the RMSE over the poses actually DRAWN, which under --keyframes is not ate_all
    # - the non-keyframe poses come from the trajectory filler's motion-only BA, not the
    # tracker's own optimisation, and labelling a keyframe picture with ate_all would misreport it
    return {'arm': arm, 'out': out, 'xyz': xyz, 'err': err, 'frames': frames,
...
def render(arms, gt_pts, ij, plane, cmap, norm, scene, path, dpi, keyframes):
    import matplotlib
    ...
    # under --keyframes ate_all is not the number on screen; name what is. 4 significant digits
    # rather than 2 decimals: a room-scale scene runs at 0.06 m and two arms would print alike
    score = (lambda a: f'kf RMSE {a["shown"]:.4g} m') if keyframes else \
        (lambda a: f'ATE {a["ate_all"]:.4g} m')
    if facet:
    ...
    cb.set_label('APE to GT (m)\nafter each arm\'s own Sim(3) alignment',
                 color=MUTED_INK, fontsize=9)
    ...
    fig.suptitle(f'{scene} — top view ({plane[0]}/{plane[1]}), '
                 ...
                 f'{"keyframes" if keyframes else "poses"}: {poses}',
...
def main():
    ...
    arms = [load_arm(args.root, args.scene, a, gt_xyz, args.keyframes) for a in args.arms]
    ...
    render(arms, gt_pts, ij, plane, ramp(args.cmap), Normalize(vmin, vmax),
           args.scene, path, args.dpi, args.keyframes)

    print(f'\n  {args.scene}  —  top view {plane[0]}/{plane[1]} ({why})')
    print(f'  GT {gt_path}  ({len(gt_pts)} poses)')
    drawn = 'keyframes' if args.keyframes else 'poses'
    print(f'  {"arm":<32}{drawn:>10}{"ATE(m)":>10}{"drawn(m)":>10}{"APE min":>10}{"max":>10}')
    print('  ' + '-' * 82)
    for a in arms:
        print(f'  {a["arm"]:<32}{len(a["err"]):>10}{a["ate_all"]:>10.4f}{a["shown"]:>10.4f}'
              f'{a["err"].min():>10.3f}{a["err"].max():>10.3f}')
    print(f'\n  colour scale {vmin:.3f} .. {vmax:.3f} m, shared by every panel'
          f'{" (--vmin/--vmax)" if args.vmin is not None or args.vmax is not None else ""}')
    print(f'  each arm carries its OWN Sim(3) alignment, so compare SHAPES, not offsets')
    print(f'  wrote {path}\n')
