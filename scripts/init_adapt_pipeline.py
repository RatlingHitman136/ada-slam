...
SCENE   = 'kitti_00'                   # names the outputs/ tree
DATA    = 'data/KITTI/00'              # preprocess_kitti.py's output layout
COLORS  = f'{DATA}/colors'             # a SYMLINK to the mirror's image_2 - nothing was copied
DEPTHS  = f'{DATA}/depths'             # velodyne GT, written by preprocess_kitti.py --with-depth
                                       ...
                                       # None here disables both and is still a valid setting
GT_TRAJ = f'{DATA}/traj_tum.txt'
CALIB   = f'{DATA}/calib.txt'
CONFIG  = 'config/kitti_config.yaml'
...
START            = 0
STOP             = 1000                # exclusive: the window is [START, STOP); None = to
...
DEPTH_PNG_SCALE  = 256.0               # metres = px / this; MUST match the dataset (TUM 6553.5).
                                       # 256 is the KITTI convention and what preprocess_kitti.py
                                       # writes; it saturates at 256 m, well past the 80 m the
                                       # velodyne returns
